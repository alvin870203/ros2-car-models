...
ARGUMENTS = [
    ...
    DeclareLaunchArgument('gazebo_control_config', default_value='virtual_robot_ackermann_gazebo_control.xacro',
                          description='The file name of the gazebo_control config, by default is virtual_robot_ackermann_gazebo_control.xacro'),
]
...
def generate_launch_description():
    ...
    spawn_wheel_joints_velocity_controller = Node(
        package='controller_manager',
        executable='spawner',
        arguments=['wheel_joints_velocity_controller', '-c', '/controller_manager'],
        output='screen',
        condition=IfCondition(AndSubstitution(use_ros2_control,
                                              PythonExpression(["'", ros2_control_config, "'",
                                                                " == 'joint_control.yaml'"]))),
    ...
    spawn_hinge_joints_position_controller = Node(
        package='controller_manager',
        executable='spawner',
        arguments=['hinge_joints_position_controller', '-c', '/controller_manager'],
        output='screen',
        condition=IfCondition(AndSubstitution(use_ros2_control,
                                              PythonExpression(["'", ros2_control_config, "'",
                                                                " == 'joint_control.yaml'"]))),
    ...
    controllers_spawn_callback = RegisterEventHandler(
        event_handler=OnProcessExit(
            ...
            on_exit=[spawn_ackermann_steering_controller,
                     spawn_wheel_joints_velocity_controller,
                     spawn_hinge_joints_position_controller],
        )
    )
